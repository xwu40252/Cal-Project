# -*- coding: utf-8 -*-
"""
Created on Tue Aug  6 10:09:10 2024
...
def process_and_plot(file_path, label, delta_u, phi, repetitions=10):
    ...
    plt.errorbar(times, mean_errors, yerr=std_errors, fmt='o', label=f'{label} - φ={phi}')
    plt.fill_between(times, mean_errors - std_errors, mean_errors + std_errors, alpha=0.2)

    for mean, std, time in zip(mean_errors, std_errors, times):
...
plt.title('Average Percentage Error % vs Time (s) with Error Bars for Different φ Values')
plt.legend()
plt.grid(True)
plt.show()
