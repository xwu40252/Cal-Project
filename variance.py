# -*- coding: utf-8 -*-
"""
Created on Tue Aug  6 10:10:19 2024
...
def process_and_plot(file_path, label, delta_u, phi, repetitions=10):
    ...
    all_errors = np.zeros((repetitions, 10))
    ...
    for rep in range(repetitions):
        ...
        all_errors[rep] = np.array(percentage_errors)
    
    mean_errors = np.mean(all_errors, axis=0)
    var_errors = np.var(all_errors, axis=0)

    times = range(60, 601, 60)
    plt.errorbar(times, mean_errors, yerr=var_errors, fmt='o', label=f'{label} - φ={phi}')
    plt.fill_between(times, mean_errors - var_errors, mean_errors + var_errors, alpha=0.2)

    for mean, var, time in zip(mean_errors, var_errors, times):
        upper_bound = mean + var
        lower_bound = mean - var
        plt.text(time + 5, upper_bound, f'{upper_bound:.2f}', ha='left', va='center', fontsize=8)
        plt.text(time + 5, lower_bound, f'{lower_bound:.2f}', ha='left', va='center', fontsize=8)
...
plt.title('Average Percentage Error % vs Time (s) with Variance Error Bars for Different φ Values')
plt.legend()
plt.grid(True)
plt.show()
